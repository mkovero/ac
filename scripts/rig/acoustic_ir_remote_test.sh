...
# config) | fail (exit 1) | lie (exit 0, config left untouched).
...
{"output_channel": 1, "input_channel": 1, "drive_max_dbfs": -40}
JSON
    # Requests 0/0 while the config starts at 1/1 (a previous run's route), so
    # "lie" leaves a real disagreement for the read-back to catch.
...
out="$(run_ir ok 2>&1)" && rc=0 || rc=$?
[[ $rc == 0 ]] || fail "setup=ok: expected exit 0, got $rc — output: $out"
[[ -f $tmp/ir_probe-ran ]] || fail "setup=ok: ir_probe should have run"
[[ $out == *"config reads output 0 / input 0, as requested"* ]] || fail "setup=ok: route not recorded — output: $out"
