...
fn parse_view_mode(s: &str) -> anyhow::Result<ViewMode> {
    match s {
        ...
        other => anyhow::bail!(
            "--view: expected spectrum|waterfall|scope|spectrum_ember|goniometer, got {other}",
        ),
    }
}
...
Keys (full list in-app: press h):\n  \
  ...
  Enter            freeze\n  \
  Tab / Sh+Tab     cycle view forward / back (page in Grid)\n  \
  g / Shift+g      toggle goniometer (active+1) / ember matrix overview\n  \
...
mod view_mode_tests {
    ...
    fn unknown_view_errors_helpfully() {
        ...
        assert!(
            err.contains("goniometer") && err.contains("spectrum_ember"),
