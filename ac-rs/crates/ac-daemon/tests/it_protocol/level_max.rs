...
//! (−20.0 dBFS) now gets `{"ok": false, ...}` with `max_dbfs` on it, never
//! a quietly-lowered success. One test per wire command listed under the
//! design's ZMQ impact item 1, at −19.9 dBFS (refused) and at exactly
//! −20.0 dBFS (accepted) — the shared `check_emission_level` arithmetic
