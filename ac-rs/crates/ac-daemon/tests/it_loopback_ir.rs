...
//! explicitly: this test's own daemon runs under an isolated `HOME` whose
//! config sets no `drive_max_dbfs`, so only the default ceiling (−10 dBFS)
//! applies to it, never the rig's own.
            ...
            (Some(output_port), Some(input_port)) => {
                ...
                let level_dbfs = level_from_env().expect(
                    "AC_LOOPBACK_OUT/IN name real JACK ports, so this run drives \
                     hardware: set AC_LOOPBACK_LEVEL_DBFS explicitly. This test's daemon \
                     runs under an isolated HOME, so only the default drive_max_dbfs \
                     (-10 dBFS) clamps it, not the rig's ceiling.",
                );
                Self {
                    output_port,
                    input_port,
