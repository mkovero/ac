...
/// end-to-end through `--fake-audio`'s new low-SNR test hooks
/// (`AC_FAKE_TAU_GAIN_OVERRIDE` / `AC_FAKE_TAU_NOISE_AMPLITUDE_OVERRIDE`,
/// `audio/fake.rs`): a muted route (gain 0, dither only) must come back
/// `not_measured_low_snr`, not the plausible-looking `measured` a deleted
/// gate would still produce, since the fake backend's default loopback
/// shape has no other codepath capable of returning anything but a clean
/// peak. Pairs with `calibrate_measures_tau_against_fake_loopback_delay`
