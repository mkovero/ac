...
mod tests {
    ...
    /// One meas/ref pair, 3 s unity-loopback-ish (distinct freqs so H1
    /// isn't trivially flat) 1 kHz/1.5 kHz tones, SPL-calibrated meas
    /// channel so `derive_pair`'s `spl` path is exercised by the fixture
    /// too, not just H1/spectra.
    #[test]
    ...
    fn generate_snapshot_fixture() {
        ...
        let meas: Vec<f32> = (0..n)
            .map(|i| {
                (0.25 * (2.0 * std::f64::consts::PI * 1_000.0 * i as f64 / sr as f64).sin()) as f32
            })
            .collect();
        let refch: Vec<f32> = (0..n)
            .map(|i| {
                (0.4 * (2.0 * std::f64::consts::PI * 1_000.0 * i as f64 / sr as f64).sin()) as f32
            })
            .collect();
        ...
        let (bytes, _) = write_acsnap(&meta, &[meas, refch]).expect("write fixture");
        std::fs::write(fixture_path(), &bytes).expect("write fixture file");
        eprintln!("wrote {} ({} bytes)", fixture_path().display(), bytes.len());
    }
    ...
    fn t3_checked_in_fixture_reprocesses_with_no_daemon() {
        ...
        // The fixture's meas/ref tones are both 1 kHz, different
        // amplitudes (0.25 vs 0.4) — H1 magnitude should reflect that
        // ratio (20*log10(0.25/0.4) ≈ -4.08 dB) at the tone frequency.
        let bin_1khz = 1000usize; // 1 Hz/bin, nperseg=sr
        let expected_db = 20.0 * (0.25_f64 / 0.4).log10();
        assert!(
            (d.h1.magnitude_db[bin_1khz] - expected_db).abs() < 0.5,
            "H1 at 1 kHz = {} dB, expected ~{expected_db:.2} dB",
            d.h1.magnitude_db[bin_1khz]
        );
    }
}
