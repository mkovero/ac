...
impl MeasurementReport {
    ...
    pub fn ir_stats(&self) -> Option<IrStats> {
        ...
        // How many samples that floor was measured over — zero means the
        // guard band ate the whole pre-peak window, which the verdict below
        // has to tell apart from a genuinely silent floor (#376).
        let pre_region_len =
            crate::measurement::sweep::pre_impulse_region_len(linear_ir.len(), peak_index);
        ...
        // (`rms == 0.0`, pre-impulse region nonempty) is the *best* possible
        ...
        // empty pre-impulse region (the guard band consumed the whole pre-peak
