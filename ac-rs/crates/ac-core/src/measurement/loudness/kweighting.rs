//! BS.1770-5 K-weighting cascade (Annex 1 §2.1 pre-filter + §2.2 RLB)
//! and the 400 ms / 100 ms-step gating-block accumulator (§2.3).
//!
//! The coefficients are re-derived at runtime from the two closed-form
//! biquad designs at the bottom of this file rather than looked up per
//! sample rate; see the module doc of [`super`] for why.
...
use super::{BLOCK_DURATION_S, BLOCK_STEP_S};
...
impl KWeighting {
    ...
    pub(super) fn process_sample(&mut self, x_in: f64) -> f64 {
        let mut x = x_in;
...
#[cfg(test)]
mod tests {
    use super::super::ms_to_lkfs;
    use super::super::test_support::FS;
    use super::*;
