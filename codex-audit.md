...
**open #637** — PR #525 (issue #435) added shared cross-field metadata validation, which rejects a role-changing swap; a swap of two same-role `per_channel` entries (multi-ref sessions write two `"ref"` channels) still passes and misbinds audio and calibration.
