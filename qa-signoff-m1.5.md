...
|---|---|---|
| AC1 Full workspace green, zero edits to pre-existing assertions (fixture-expectation update sanctioned) | ✓ | 543 passed, 2 ignored (JACK runbook, fixture regenerator), 0 failed — 3 consecutive full-workspace runs (one transient flake in the unrelated, untouched `test_software` module reproduced as green on isolation + 3 retries — same pre-existing flake class seen during M0's and M1's QA passes). `git diff main` confirms the *only* deletions are inside `generate_snapshot_fixture`/`t3_checked_in_fixture_reprocesses_with_no_daemon` (fixture content + its hand-derived expectation) plus two small non-assertion edits in `fake.rs` (a doc comment and an or-pattern match arm extending exhaustiveness for the new enum variant) — no other test assertion touched anywhere. |
